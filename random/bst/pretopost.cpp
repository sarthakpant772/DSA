...
int ind=0;
Node* construct(int pre[],int size,int mi,int ma){
    
    if(ind<size && pre[ind]>=mi && pre[ind]<=ma){
        Node* node=newNode(pre[ind++]);
        
        node->left=construct(pre,size,mi,node->data-1);
        node->right=construct(pre,size,node->data+1,ma);
        return node;
    }
    return NULL;
}
